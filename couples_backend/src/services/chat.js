...
function validateTextContent(content, { allowEmpty = false } = {}) {
  ...
  if (trimmedContent.length > config.messageMaxLength) {
    throw new AppError(
      'content_too_long',
      `content exceeds max length ${config.messageMaxLength}`,
    );
  }
  return trimmedContent;
}

function ensureSenderInCouple(db, { coupleId, senderUserId }) {
  const couple = db
...
function normalizeMessageType(messageType) {
  ...
  switch (raw) {
    ...
    default:
      throw new AppError(
        'invalid_request',
        'messageType must be one of text, image or voice',
      );
  }
}
...
function uploadChatMedia(
  ...
  if (!trimmedCoupleId || !trimmedSenderUserId || !trimmedBase64) {
    throw new AppError(
      'invalid_request',
      'coupleId, senderUserId and fileBytesBase64 are required',
    );
  }

  ensureSenderInCouple(db, {
  ...
  const extension = normalizeFileExtension(
    trimmedFileName,
    normalizedMediaKind === 'image' ? '.jpg' : '.m4a',
  );
  const generatedName =
    `${normalizedMediaKind}-${Date.now()}-${Math.floor(Math.random() * 9000) + 1000}${extension}`;
  ...
  const absoluteFilePath = path.join(absoluteDir, generatedName);
...
function sendChatMessage(
  ...
  const trimmedContent = validateTextContent(content, {
    allowEmpty: normalizedMessageType !== 'text',
  });

  if (!trimmedCoupleId || !trimmedSenderUserId || !trimmedClientMessageId) {
    throw new AppError(
      'invalid_request',
      'coupleId, senderUserId and clientMessageId are required',
    );
  }
  ...
  if (normalizedMessageType !== 'text' && !trimmedMediaUrl) {
    throw new AppError(
      'invalid_request',
      'mediaUrl is required for image or voice messages',
    );
  }
  ...
  if (normalizedMessageType === 'voice' && normalizedMediaDurationMs <= 0) {
    throw new AppError(
      'invalid_request',
      'mediaDurationMs must be greater than 0 for voice messages',
    );
  }
      ...
      .prepare(
        `
          SELECT
            ...
            created_at
          FROM chat_messages
          WHERE couple_id = ? AND client_message_id = ?
        `,
      )
      .get(trimmedCoupleId, trimmedClientMessageId);
...
function listChatMessages(db, { coupleId, since }) {
  const trimmedCoupleId = String(coupleId || '').trim();
  if (!trimmedCoupleId) {
